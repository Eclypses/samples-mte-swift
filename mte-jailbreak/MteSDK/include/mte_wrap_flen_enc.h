...
SOFTWARE.
*/
#ifndef mte_wrap_flen_enc_h
#define mte_wrap_flen_enc_h

#ifndef mte_wrap_base_h
#include "mte_wrap_base.h"
#endif

/* This wraps the MTE fixed-length add-on encoder functions.
...
/* Returns the encoder state size for the given DRBG algorithm, token size in
   ...
   usable. */
MTE_SHARED
MTE_UINT32_T mte_wrap_flen_enc_state_bytes(mte_drbgs drbg,
...
/* Initialize the encoder state given the DRBG algorithm, token size in bytes,
   ...
   mte_wrap_flen_enc_state_bytes(). */
MTE_SHARED
mte_status mte_wrap_flen_enc_state_init(MTE_HANDLE state,
                                        mte_drbgs drbg,
                                        MTE_UINT32_T tok_bytes,
                                        mte_verifiers verifiers,
                                        MTE_UINT32_T fixed_bytes);
...
MTE_SHARED
mte_status mte_wrap_flen_enc_instantiate(MTE_HANDLE state,
...
MTE_SHARED
MTE_UINT32_T mte_wrap_flen_enc_save_bytes(MTE_CHANDLE state);

/* Returns the state save size [Base64-encoded]. Returns 0 if save is
   unsupported. */
MTE_SHARED
MTE_UINT32_T mte_wrap_flen_enc_save_bytes_b64(MTE_CHANDLE state);

/* Save the encoder state to the given buffer encoded in Base64. The size of
   the buffer must be mte_wrap_flen_enc_save_bytes_b64() and the result is null-
   terminated. Returns mte_status_unsupported if not supported; otherwise
   returns mte_status_success. */
MTE_SHARED
mte_status mte_wrap_flen_enc_state_save_b64(MTE_CHANDLE state, void *saved);
...
/* Restore the encoder state from the given buffer in raw form. Returns
   ...
   mte_status_success. */
MTE_SHARED
mte_status mte_wrap_flen_enc_state_restore_b64(MTE_HANDLE state,
                                               const void *saved);

/* Returns the encode buffer size [raw] in bytes. The encode buffer provided
   to mte_wrap_flen_enc_encode() must be of at least this length. */
MTE_SHARED
MTE_UINT32_T mte_wrap_flen_enc_buff_bytes(MTE_CHANDLE state);

/* Returns the encode buffer size [Base64-encoded] in bytes. The encode buffer
   provided to mte_wrap_flen_enc_encode_b64() must be of at least this length. */
MTE_SHARED
MTE_UINT32_T mte_wrap_flen_enc_buff_bytes_b64(MTE_CHANDLE state);
...
/* Encode the given data of the given byte length to the given encode buffer in
   ...
   See mte_wrap_flen_enc_buff_bytes(). */
MTE_SHARED
mte_status mte_wrap_flen_enc_encode(MTE_HANDLE state,
...
/* Encode the given data of the given byte length to the given encode buffer,
   ...
   *encoded_bytes to the Base64-encoded version length in bytes. The encoded
   ...
   The encoded buffer must be of sufficient length to hold the encoded version.
   See mte_wrap_flen_enc_buff_bytes_b64(). */
MTE_SHARED
mte_status mte_wrap_flen_enc_encode_b64(MTE_HANDLE state,
                                        mte_verifier_get_timestamp64 t_cb,
                                        void *t_cb_context,
                                        const void *data,
